...
class MyForm(FlaskForm):
    ...
    lines = RadioField('lines', choices=[('6','6 （大）'),('8','8 （中）'),('10','10 （小）')])
...
def create_pdf(my_canvas, title, lines_per_page, text):
    ...
    fontsize = 315 / int(lines_per_page)
    ...
    for p in range(1, number_of_pages+1):
        ...
        ### Write logo
        my_canvas.drawImage('bookworm.jpg', 500, 700, width=70,
        ...
        # initialize x and y
        y = A4[1] - (line_spacing * 7)

        # fetch lines index
        first_line_idx = (p-1)*(int(lines_per_page))+1
        last_line_idx = (p-1)*(int(lines_per_page))+int(lines_per_page)+1
